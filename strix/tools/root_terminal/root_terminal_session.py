"""
Root Terminal Session - Individual root-accessed terminal session.

This module provides a root-privileged terminal session with full system access
for installing tools, libraries, databases, and executing privileged commands.
"""
...
class RootBashCommandStatus(Enum):
    """Status of a root bash command execution."""
    CONTINUE = "continue"
    COMPLETED = "completed"
...
def _remove_command_prefix(command_output: str, command: str) -> str:
    """Remove command prefix from output."""
    return command_output.lstrip().removeprefix(command.lstrip()).lstrip()


class RootTerminalSession:
    """
    Root-privileged terminal session with full system access.
    
    Features:
    - Root/sudo access by default
    ...
    POLL_INTERVAL = 0.5
    HISTORY_LIMIT = 15_000  # Larger history for root operations
    PS1_END = "]# "  # Root prompt indicator
    ...
    def __init__(self, session_id: str, work_dir: str = "/workspace") -> None:
        ...
        self.prev_status: RootBashCommandStatus | None = None
    ...
    def PS1(self) -> str:  # noqa: N802
        """Custom prompt for tracking command completion."""
        return r"[STRIX_ROOT_$?]# "
    
    @property
    def PS1_PATTERN(self) -> str:  # noqa: N802
        """Pattern to match the custom prompt."""
        return r"\[STRIX_ROOT_(\d+)\]"
    
    def initialize(self) -> None:
        """Initialize the root terminal session."""
        self.server = libtmux.Server()
        
        session_name = f"strix-root-{self.session_id}-{uuid.uuid4()}"
        self.session = self.server.new_session(
            session_name=session_name,
            start_directory=self.work_dir,
            kill_session=True,
            x=160,  # Wider terminal for root operations
            y=50,   # Taller terminal
        )
        ...
        # Set up root prompt and environment
        self.pane.send_keys(f'export PROMPT_COMMAND=\'export PS1="{self.PS1}"\'; export PS2=""')
        time.sleep(0.1)
        
        # Configure sudo to not require password for current user (passwordless sudo)
        # This enables true root operations
        self.pane.send_keys('export SUDO_ASKPASS=/bin/false')
        time.sleep(0.05)
        
        # Set PATH to include common tool directories
        self.pane.send_keys('export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:$PATH"')
        time.sleep(0.05)
    ...
    def _get_pane_content(self) -> str:
        ...
        if not self.pane:
            raise RuntimeError("Root terminal session not properly initialized")
        return "\n".join(
    ...
    def _is_control_key(self, command: str) -> bool:
        """Check if command is a control key."""
        return (
    ...
    def _is_function_key(self, command: str) -> bool:
        """Check if command is a function key."""
    ...
    def _is_navigation_or_special_key(self, command: str) -> bool:
        """Check if command is a navigation or special key."""
    ...
    def _is_complex_modifier_key(self, command: str) -> bool:
        """Check if command uses complex modifiers."""
    ...
    def _is_special_key(self, command: str) -> bool:
        """Check if command is a special key combination."""
    ...
    def _matches_ps1_metadata(self, content: str) -> list[re.Match[str]]:
        """Find all PS1 prompt matches in content."""
    ...
    def _get_command_output(
        ...
        """Extract command output from raw terminal content."""
        if self.prev_output:
    ...
    def _extract_exit_code_from_matches(self, ps1_matches: list[re.Match[str]]) -> int | None:
        """Extract exit code from PS1 prompt matches."""
        if not ps1_matches:
            return None
    ...
    def _handle_empty_command(
        ...
        """Handle execution of empty command (get current output)."""
        ...
        while True:
            ...
            if cur_pane_output.rstrip().endswith(self.PS1_END.rstrip()) or len(ps1_matches) > 0:
                ...
                command_output = self._get_command_output("", raw_command_output)
                self.prev_status = RootBashCommandStatus.COMPLETED
                self.prev_output = ""
            ...
            if elapsed_time >= timeout:
                ...
                return {
                    "content": command_output
                    + f"\n[Root command still running after {timeout}s - showing output so far]",
                    "status": "running",
                    "exit_code": None,
                    "working_dir": self._cwd,
                }
    ...
    def _handle_input_command(
        ...
        """Handle input to a running command."""
        if not is_command_running:
            return {
        ...
        while True:
            ...
            if elapsed_time >= timeout:
                ...
                command_output = self._get_command_output(
                    command,
                    raw_command_output,
                    continue_prefix="[Below is the output of the previous root command.]\n",
                )
                self.prev_status = RootBashCommandStatus.CONTINUE
    ...
    def execute(
        self, command: str, is_input: bool = False, timeout: float = 60.0, no_enter: bool = False
    ) -> dict[str, Any]:
        """Execute a command in the root terminal."""
        if not self._initialized:
            raise RuntimeError("Root bash session is not initialized")
        ...
        if is_command_running:
            return {
                "content": (
                    "A root command is already running. Use is_input=true to send input to it, "
    ...
    def is_running(self) -> bool:
        ...
        try:
            ...
            logger.debug("Error checking if root session is running: %s", e)
            return False
    ...
    def close(self) -> None:
        ...
        if self.session:
            try:
                ...
                logger.debug("Error closing root terminal session: %s", e)
